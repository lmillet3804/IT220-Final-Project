...
function randomWords(): [string, string, string] {
  const picked = shuffle([...wordList]).slice(0, 3);
  return [picked[0], picked[1], picked[2]];
}

function randomStoryId(game: GameState): string {
  const index = Math.floor(Math.random() * game.stories.length);
  return game.stories[index].id;
}
...
function fillMissingWriteSubmissions(game: GameState): void {
  for (const [playerId, assignment] of Object.entries(game.writeAssignments)) {
    if (game.writeSubmissions[assignment.lineId]) {
      continue;
    }
...
function fillMissingGuessSubmissions(game: GameState): void {
  for (const [guesserId, assignment] of Object.entries(game.guessAssignments)) {
    if (game.guessSubmissions[assignment.lineId]) {
      continue;
    }

    game.guessSubmissions[assignment.lineId] = {
      guesserId,
      guessedStoryId: randomStoryId(game),
      submittedAt: now(),
      autoGenerated: true,
    };
  }
...
function finalizeRound(game: GameState): void {
  for (const [lineId, writeSubmission] of Object.entries(
    ...
    line.history.push({
      ...
      timeoutNote:
        writeSubmission.autoGenerated && guessSubmission?.autoGenerated
          ? "Write and guess were auto-generated after timeout."
          : writeSubmission.autoGenerated
            ? "Words were auto-generated after write timeout."
            : guessSubmission?.autoGenerated
              ? "Story guess was auto-generated after guess timeout."
              : undefined,
    });
...
function tickTimeouts(game: GameState): void {
  ...
  if (game.phase === "write") {
    fillMissingWriteSubmissions(game);
    assignGuessPhase(game);
    return;
  }

  if (game.phase === "guess") {
    fillMissingGuessSubmissions(game);
    finalizeRound(game);
  }
}
